...
PercOperatorModulator:Array[0..10] of Byte = (
      $f2, $53, $74, $00, $00, $08,
      $10, $14, $12, $15, $11);
...
TAdlib = Object(TObject)
 Public
  ...
  MaxVoices: Integer;                  (* maximum number of available voices *)
...
Begin
 ...
 WriteLn('Carte Adlib non trouv�e. Arr�t du programme.');
 Halt(1);
end;
...
Begin
  ...
  opOffset := PercOperatorModulator[BassDrumVoice];
  with BassDrum[Modulator] do begin
    SndOutput(opOffset + $20, WaveChar);
    SndOutput(opOffset + $40, ScalingOutput);
    SndOutput(opOffset + $60, AttackDecay);
    SndOutput(opOffset + $80, SustainRelease);
    SndOutput(BassDrumVoice + $C0, Feedback);
    SndOutput(opOffset + $E0, SetWaveSelect);
  end;

  with BassDrum[Carrier] do begin
    SndOutput(opOffset + $23, WaveChar);
    SndOutput(opOffset + $43, ScalingOutput);
    SndOutput(opOffset + $63, AttackDecay);
    SndOutput(opOffset + $83, SustainRelease);
    SndOutput(opOffset + $E3, SetWaveSelect);
  end;
  ...
  Begin
    ...
    opOffset := PercOperatorModulator[i];
    SndOutput(opOffset + $20, InstrArray[i].WaveChar);
    SndOutput(opOffset + $40, InstrArray[i].ScalingOutput);
    SndOutput(opOffset + $60, InstrArray[i].AttackDecay);
    SndOutput(opOffset + $80, InstrArray[i].SustainRelease);
    SndOutput(i + $C0, InstrArray[i].Feedback);
    SndOutput(opOffset + $E0, InstrArray[i].SetWaveSelect);
  end;
end;
...
Var
  Voice: Byte;
  Index: Byte;
  operatorArg: Byte; { 0: modulator, 1: carrier }
Begin
  (* Optimized greatly for size this routine, everything is now in a *)
  (* tight loop.                                                     *)
  For Voice:=0 to MaxVoices do
  Begin
    For operatorArg := 0 to 1 do
    Begin
      If operatorArg = 0 then Index := 0 else Index := 3;

      SndOutput(TableModulator[Voice] + $20 + Index,
        ElectricPiano[operatorArg].WaveChar);

      SndOutput(TableModulator[Voice] + $40 + Index,
        ElectricPiano[operatorArg].ScalingOutput);

      SndOutput(TableModulator[Voice] + $60 + Index,
        ElectricPiano[operatorArg].AttackDecay);

      SndOutput(TableModulator[Voice] + $80 + Index,
        ElectricPiano[operatorArg].SustainRelease);

      SndOutput(TableModulator[Voice] + $E0,
        ElectricPiano[operatorArg].SetWaveSelect);
    end;
    ...
    ModulatorScalingLevel[Voice] :=
      ElectricPiano[Modulator].ScalingOutput AND $C0;
...
Begin
 ASM
  ...
  Begin
   ...
   Begin
     SndOutput(PercOperatorModulator[Voice] + $43, ValueOne);
   end
   else
     { These percussive voices only have one operator each, write }
     { the volume level directly.                                 }
     SndOutput(PercOperatorModulator[Voice] + $40, ValueOne);
  end;
end;
